...
from .bi_encoder_losses import BiEncoderLoss
...
class SparseBiNegativeCELoss(SparseBiEncoderModule):
    ...
    def forward(
        ...
        total_loss = contrastive_loss + reg_d + reg_q
...
class SparseM2BiNegativeCELoss(SparseBiEncoderModule):
    """
    SPLADE-style contrastive loss with hard negatives + in-batch InfoNCE + FLOPs regularization.
...
class SparseMLMBiNegativeCELoss(SparseBiEncoderModule):
    """
    SPLADE-style contrastive loss with hard negatives + in-batch InfoNCE + FLOPs regularization.
    ...
    def __init__(
        ...
        # Avoid module instances as default args; create them per-loss instance.
        self.q_regularizer = q_regularizer or FLOPs(weight=0.01, T=10000)
        self.d_regularizer = d_regularizer or FLOPs(weight=0.001, T=10000)
