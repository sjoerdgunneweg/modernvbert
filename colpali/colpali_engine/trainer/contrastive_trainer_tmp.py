...
class ContrastiveTrainer(Trainer):
    def __init__(self, loss_func, is_vision_model, compute_symetric_loss=False, *args, **kwargs):
        ...
        # --- NEW: initialize prefixes so compute_loss always has them ---
        self._init_prefixes_from_collator(self.data_collator)
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        if is_datasets_available() and isinstance(dataset, datasets.Dataset):
            # print("Removing unused columns from dataset...")
    ...
    def _compute_loss_from_outputs(
        ...
        if isinstance(loss, dict):
            return loss  # return full dict — compute_loss will handle it
        else:
            return {"loss": loss}  # wrap scalar
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        # Ensure prefixes exist even if something weird happened before
        if not hasattr(self, "query_prefix") or not hasattr(self, "pos_prefix") or not hasattr(self, "neg_prefix"):
            self._init_prefixes_from_collator(self.data_collator)
        ...
        return (loss['loss'], (query_outputs, doc_outputs)) if return_outputs else loss['loss']
    ...
    def _init_prefixes_from_collator(self, data_collator):
        """
        Ensure query/doc/neg prefixes always exist, even if get_train_dataloader
        has not been called yet.
        """
        # Fallback defaults are what your naming convention suggests
        self.query_prefix = getattr(data_collator, "query_prefix", "query_")
        self.pos_prefix = getattr(data_collator, "pos_doc_prefix", "doc_")
        self.neg_prefix = getattr(data_collator, "neg_doc_prefix", "neg_doc_")
