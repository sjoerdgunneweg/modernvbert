...
To run the contrastive training experiments, use the job files provided in the ```job_files``` directory. These are written as Slurm jobs, but the commands inside them can also be executed directly without Slurm.

To train the sparse models ```SparseModernVBERTM2``` and ```SparseModernVBERTMLM```, you first need base models with the ```ModernVBERT``` weights loaded in the correct order. These can be generated by running the ```make_cpt.py``` script. Be sure to update the destination paths inside the script before executing it.
